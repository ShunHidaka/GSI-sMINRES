...
int main(int argc, char* argv[]) {
  ...
  Eigen::Map<Eigen::VectorXcd> wv(w.data(), static_cast<Eigen::Index>(N));
  Eigen::Map<Eigen::VectorXcd> uv(u.data(), static_cast<Eigen::Index>(N));
  // Prepare Eigen CG solver
  Eigen::ConjugateGradient<eigen_bridge::SpMatRow,
                           Eigen::Lower | Eigen::Upper,
                           //Eigen::IdentityPreconditioner
                           //Eigen::DiagonalPreconditioner<eigen_bridge::Scalar>
                           Eigen::IncompleteCholesky<eigen_bridge::Scalar>
                           > cg;
  cg.setMaxIterations(static_cast<Eigen::Index>(10*N));
  cg.setTolerance(1e-12);
  cg.compute(B_eigen_map);
  if (cg.info() != Eigen::Success) {
    std::cerr << "Eigen CG compute(B) failed" << std::endl;
    return 1;
  }
  ...
  wv = cg.solve(bv);
  inner_iters = static_cast<std::size_t>(cg.iterations()); inner_relres = cg.error();
  ...
  std::cout << "# gs-MINRES method (IC-CG)" << std::endl;
  std::cout << "# A = " << Aname << "\n"
            << "# B = " << Bname << std::endl;
