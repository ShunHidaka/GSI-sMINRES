...
namespace gsi_sminres {
  namespace shift_invert {
    ...
    void Solver::initialize_r(std::vector<std::complex<double>>& x,
      ...
      omega_ = omega;
      rtol_ = rtol;
      r0_norm_ = std::sqrt( std::max(0.0, linalg::blas::ddot(matrix_size_, v, 0, Bv, 0)) );
      if (r0_norm_ < rtol) {
        for (std::size_t m = 0; m < shift_size_; ++m) is_conv_[m] = 1u;
        conv_num_ = shift_size_;
        std::fill(h_.begin(), h_.end(), r0_norm_);
      ...
      linalg::blas::dscal(matrix_size_, 1.0/r0_norm_,  v);
      linalg::blas::dscal(matrix_size_, 1.0/r0_norm_, Bv);
      linalg::blas::dcopy(matrix_size_, v, 0, v_curr_r_, 0);
    ...
    void Solver::sislanczos_pre_r(std::vector<double>& v,
                                  const std::vector<double>& Bv) noexcept {
      alpha_ = linalg::blas::ddot(matrix_size_, Bv, 0, v, 0);
    ...
    void Solver::sislanczos_pst_r(std::vector<double>& v,
                                  std::vector<double>& Bv) noexcept {
      beta_curr_ = std::sqrt( linalg::blas::ddot(matrix_size_, v, 0, Bv, 0) );
      linalg::blas::dscal(matrix_size_, 1.0/beta_curr_, v);
      linalg::blas::dscal(matrix_size_, 1.0/beta_curr_, Bv);
...
#ifdef GSI_SMINRES_USE_OMP_UPDATE
#pragma omp parallel for reduction(+:local_conv_num)
      ...
      for (std::size_t m = 0; m < shift_size_; ++m) {
        ...
        std::complex<double> T_prev2 = 0.0;
        std::complex<double> T_prev  = (sigma_[m] - omega_) * beta_prev_;
        std::complex<double> T_curr  = 1.0 + (sigma_[m] - omega_) * alpha_;
        std::complex<double> T_next  = (sigma_[m] - omega_) * beta_curr_;
        if (iter_ >= 3) {
          linalg::blas::apply_givens(Gc_[m][0], Gs_[m][0], T_prev2, T_prev);
        }
        if (iter_ >= 2) {
          linalg::blas::apply_givens(Gc_[m][1], Gs_[m][1], T_prev, T_curr);
        }
        linalg::blas::zrotg(T_curr, T_next, Gc_[m][2], Gs_[m][2]);
        //linalg::lapack::zlartg(T_curr, T_next, Gc_[m][2], Gs_[m][2]);
        std::size_t offset = m*matrix_size_;
        linalg::blas::zcopy(matrix_size_, v_curr_, 0, p_curr_, offset);
        linalg::blas::zaxpy(matrix_size_, -T_prev2, p_prev2_, offset, p_curr_, offset);
        linalg::blas::zaxpy(matrix_size_, -T_prev,  p_prev_,  offset, p_curr_, offset);
        linalg::blas::zscal(matrix_size_, 1.0/T_curr, p_curr_, offset);
